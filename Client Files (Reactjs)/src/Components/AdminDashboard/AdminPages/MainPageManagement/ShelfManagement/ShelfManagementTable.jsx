...
import $ from "jquery";
import "./ShelfManagementTable.css";
class ShelfManagementTable extends React.Component {
  componentDidMount() {
      ...
      >
        ...
        <tbody>
          {datas.map((data) => {
            return (
              <tr>
                <td>{data.shelf.name}</td>
                <td>
                  <a
                    onClick={() => this.props.loadUpdateData(data.shelf.id)}
                  ...
                  &nbsp;
                  <a
                    onClick={() => this.props.removeItem(data.shelf.id)}
